<template>
  <div class="ad-edit-three">
    <div class="content-container">
      <span>
        <div class="title md-caption">Ad Image 3</div>
...
export default {
  ...
  mounted () {
    this.$watch('activeLanguageCode', () => {
      db.ref('home').child(store.getters.activeLanguageCode).child('three').once('value', snapshot => {
        if (snapshot.exists()) {
  ...
  methods: {
    ...
    updateAd () {
      db.ref('home').child(store.getters.activeLanguageCode).child('three').set(this.ad)
      this.$router.go(-1)
    },
    selectFile () {
    ...
    upload (file) {
      ...
      this.uploading = true
      this.uploadTask = firestorage.ref('home/' + store.getters.activeLanguageCode + '/ad/three/' + file.name).put(file)
    },
    revertImage (navigate) {
      firestorage
        .ref('home/' + store.getters.activeLanguageCode + '/ad/three/' + this.fileName)
        .delete()
        .then(() => {
          this.fileName = ''
