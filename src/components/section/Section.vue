    ...
    sectionListSorted: {
      ...
      set (value) {
        ...
        value.forEach((item, index) => {
          db.ref(store.getters.activeLanguageCode).child('section').child(this.sectionName).child('items').on('value', function (snapshot) {
            updates[item['.key'] + '/order'] = index
          })
        })

        db.ref(store.getters.activeLanguageCode).child('section').child(this.sectionName).child('items').update(updates)
      }
    }
  },
  mounted () {
    this.$watch('sectionName', () => {
      this.$bindAsArray('sectionList', db.ref(store.getters.activeLanguageCode).child('section').child(this.sectionName).child('items').orderByChild('order'))
    }, {
      immediate: true
    })
