<template>
  <div class="ad-edit-one">
    <div class="content-container">
      <span>
        <div class="title md-caption">Ad Image 1</div>
...
export default {
  ...
  mounted () {
    this.$watch('activeLanguageCode', () => {
      db.ref('home').child(store.getters.activeLanguageCode).child('one').once('value', snapshot => {
        if (snapshot.exists()) {
  ...
  methods: {
    ...
    updateAd () {
      db.ref('home').child(store.getters.activeLanguageCode).child('one').set(this.ad)
      this.$router.go(-1)
    },
    selectFile () {
    ...
    upload (file) {
      ...
      this.uploading = true
      this.uploadTask = firestorage.ref('home/' + store.getters.activeLanguageCode + '/ad/one/' + file.name).put(file)
    },
    revertImage (navigate) {
      firestorage
        .ref('home/' + store.getters.activeLanguageCode + '/ad/one/' + this.fileName)
        .delete()
        .then(() => {
          this.fileName = ''
