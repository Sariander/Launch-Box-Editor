<template>
  <div class="ad-edit-four">
    <div class="content-container">
      <span>
        <div class="title md-caption">Ad Image 4</div>
...
export default {
  ...
  mounted () {
    this.$watch('activeLanguageCode', () => {
      db.ref('home').child(store.getters.activeLanguageCode).child('four').once('value', snapshot => {
        if (snapshot.exists()) {
  ...
  methods: {
    ...
    updateAd () {
      db.ref('home').child(store.getters.activeLanguageCode).child('four').set(this.ad)
      this.$router.go(-1)
    },
    selectFile () {
    ...
    upload (file) {
      ...
      this.uploading = true
      this.uploadTask = firestorage.ref('home/' + store.getters.activeLanguageCode + '/ad/four/' + file.name).put(file)
    },
    revertImage (navigate) {
      firestorage
        .ref('home/' + store.getters.activeLanguageCode + '/ad/four/' + this.fileName)
        .delete()
        .then(() => {
          this.fileName = ''
